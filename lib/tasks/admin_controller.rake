namespace :admin do
  ...
  task :controller, [ :model_name ] => :environment do |t, args|
    ...
    unless model_name
      puts "Usage: rake admin:controller[ModelName]"
      puts "Example: rake admin:controller[Boat]"
      exit 1
    end
    ...
    command = "bin/rails generate houston_cms:admin_scaffold_controller #{model_name} #{attr_strings.join(' ')} --model-name=#{model_name}"
    puts "Running: #{command}"
    system(command)
  rescue NameError
    puts "Model #{model_name} not found. Make sure it exists first."
    exit 1
  end
end
